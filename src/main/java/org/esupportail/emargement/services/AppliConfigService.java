...
public class AppliConfigService {
	...
	enum AppliConfigKey {
		...
		BEFORE_START_EMARGER_LINK, ADE_SURVEILLANT, ADE_PROJET, ADE_CATEGORIES, ADE_IMPORT_MEMBERS, ADE_IMPORT_SOURCE_PARTICIPANTS, ADE_IMPORT_AFFICHER_GROUPES,
		...
		ADE_INSTRUCTOR_MANAGER, PARTICIPANT_TERM, ADE_CHECK_ORPHANS, DISPLAY_SESSION_GROUPS, DISPLAY_TAGCHECKERS_LIST, ENABLE_TROMBIVIEW
	}
  ...
  public Boolean isSessionGroupsDisplayed() {
	...
	public Boolean isTagCheckersListDisplayed() {
		AppliConfig appliConfig = getAppliConfigByKey(AppliConfigKey.DISPLAY_TAGCHECKERS_LIST);
		return appliConfig!=null && "true".equalsIgnoreCase(appliConfig.getValue());	
	}
	
	public Boolean isTrombiViewEnabled() {
		AppliConfig appliConfig = getAppliConfigByKey(AppliConfigKey.ENABLE_TROMBIVIEW);
		return appliConfig!=null && "true".equalsIgnoreCase(appliConfig.getValue());	
	}
