...
public class AppliConfigService {
	...
	public String getAdeImportSourceParticipants() {
		AppliConfig appliConfig = getAppliConfigByKey(AppliConfigKey.ADE_IMPORT_SOURCE_PARTICIPANTS);
		String source = appliConfig==null ? "" : appliConfig.getValue();
		if (0 == source.length()) {
			// Pour compatibilité
			if (isMembersAdeImport()) {
				source = "ade";
			} else {
				source = "aucune";
			}
